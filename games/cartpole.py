...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Training
        # The results_path, as he's initialized it, automatically creates a "results" directory, a directory for this game's results, and a directory for the checkpoint. Really cool!
        self.results_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../results", os.path.basename(__file__)[:-3], datetime.datetime.now().strftime("%Y-%m-%d--%H-%M-%S"))  # Path to store the checkpoints, aka the model weights and TensorBoard logs
